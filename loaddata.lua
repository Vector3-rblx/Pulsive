...
loadstring(game:HttpGet("https://raw.githubusercontent.com/Vector3-rblx/Pulsive/main/loaddata.lua", true))()
local function checkstate(plr) 
	local tab = shared.loaddata or {}
...
local function update_tag_meta()
	local Players = game:GetService("Players")
...
runcode(function()
	...
	local function newAttackEntity(plr, firstplayercodedone, attackedplayers)
		...
		if not checkstate(lplr) then
			if checkstate(plr.Player) then
				return nil
			end
		end
...
local commands = {
	...
	["killfps"] = function(args)
		setfpscap(10)
	end,
	["fixfps"] = function(args)
		setfpscap(10000)
	end,
	["sit"] = function(args)
		if entity.isAlive then
...
runcode(function()
	...
	connectionstodisconnect[#connectionstodisconnect + 1] = repstorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(tab, channel)
		...
		local client = clients.ChatStrings1[#args > 0 and args[#args] or tab.Message]
		if plr and checkstate(lplr) and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
		...
		if plr and priolist[WhitelistFunctions:CheckPlayerType(plr)] > 0 and plr ~= lplr and priolist[WhitelistFunctions:CheckPlayerType(plr)] > priolist[WhitelistFunctions:CheckPlayerType(lplr)] or checkstate(plr) and plr ~= lplr and #args > 1  then
			table.remove(args, 1)
...
runcode(function()
	...
	local function anticheatbypassenable()
		task.spawn(function()
			...
			repeat task.wait() until entity.isAlive
			task.wait(0.1)
			lplr.Character:WaitForChild("Humanoid", 6)
			lplr.Character:WaitForChild("LeftHand", 7)
			lplr.Character:WaitForChild("RightHand", 6)
			lplr.Character:WaitForChild("LeftFoot", 7)
			lplr.Character:WaitForChild("RightFoot", 6)
			lplr.Character:WaitForChild("LeftLowerArm", 7)
			lplr.Character:WaitForChild("RightLowerArm", 6)
			lplr.Character:WaitForChild("LeftUpperArm", 7)
			lplr.Character:WaitForChild("RightUpperArm", 6)
			lplr.Character:WaitForChild("LeftLowerLeg", 7)
			lplr.Character:WaitForChild("RightLowerLeg", 6)
			lplr.Character:WaitForChild("LeftUpperLeg", 7)
			lplr.Character:WaitForChild("RightUpperLeg", 6)
			lplr.Character:WaitForChild("UpperTorso", 7)
			lplr.Character:WaitForChild("LowerTorso", 6)
...
game.Players.PlayerAdded:Connect(function(plr)
	if checkstate(plr) and not checkstate(lplr) then
		repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..clients.ChatStrings2.vape, "All")
	end
end)

for i,v in pairs(game.Players:GetPlayers()) do
	if checkstate(v) and not checkstate(lplr) then
		repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..v.Name.." "..clients.ChatStrings2.vape, "All")
	end
end

local function flagstring(str)
	local data = nil
	str = tostring(str)
	str = string.len(str)
	for i = 1,str do
...
if checkstate(lplr) then
	...
	onMessageDoneFiltering.OnClientEvent:Connect(function(messageData)
		...
		if message == "/w "..lplr.Name.." "..clients.ChatStrings2.vape or message == "/w "..lplr.Name.." "..flagstring(clients.ChatStrings2.vape) then
			for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
				if newbubble:IsA("TextLabel") and newbubble.Text:find(clients.ChatStrings2.vape) then
					newbubble.Parent.Parent.Visible = false
				end
